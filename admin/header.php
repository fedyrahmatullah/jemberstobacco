<meta http-equiv="content-type" content="text/html; charset=UTF-8">
  <title> JEMBER's Tobacco </title>

     <meta name="viewport" content="width=device-width, initial-scale=1">

     <meta name="description" content="Jurusan Teknologi Informasi Politeknik Negeri Jember">
     <meta name="keywords" content="jti, polije,">
     <meta name="author" content="Sambutan Ketua Jurusan Teknologi Informasi">

     <!-- URL Theme Color for Chrome, Firefox OS, Opera and Vivaldi -->
   <meta name="theme-color" content="#2C3E50">
   <!-- URL Theme Color for Windows Phone -->
   <meta name="msapplication-navbutton-color" content="#2C3E50">
   <!-- URL Theme Color for iOS Safari -->
   <meta name="apple-mobile-web-app-capable" content="yes">

<meta name="apple-mobile-web-app-status-bar-style" content="#F0E17C">
     <meta property="og:url" content="http://jti.polije.ac.id/dosen">
     <meta property="og:title" content=" Alumni JTI ">
     <meta property="og:description" content="Jurusan Teknologi Informasi Politeknik Negeri Jember">
     <meta property="og:site_name" content="Jurusan Teknologi Informasi">
     <meta property="og:image" content="http://jti.polije.ac.id/an-component/media/upload-gambar-pendukung/Gedung%20JTI.png">
     <meta property="og:image:type" content="image/jpeg">
     <meta property="og:type" content="website">


  <link rel="shortcut icon" href="img_admin/logodaun.png">  
  <link href="../css/css_002.css" rel="stylesheet" type="text/css">
  <link href="../css/css_003.css" rel="stylesheet" type="text/css">
  <link href="../css/css_004.css" rel="stylesheet" type="text/css"> 
  <link href="../css/css.css" rel="stylesheet" type="text/css">

  <!--<link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css" /> -->
  <link rel="stylesheet" type="text/css" href="../css/font-awesome.css">
  <link rel="stylesheet" type="text/css" href="../css/ionicons.css">


  <!--<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.css" />-->
  <link rel="stylesheet" type="text/css" href="../css/jquery_003.css">
  <link rel="stylesheet" type="text/css" href="../css/jquery_004.css">
  <link rel="stylesheet" type="text/css" href="../css/jquery.css">


  <link rel="stylesheet" type="text/css" href="../css/jquery_002.css"> 



  <!--<link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css" />-->
  <link rel="stylesheet" type="text/css" href="../css/bootstrap.css">


  <!--<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/owl-carousel/1.3.3/owl.carousel.min.css" />-->
  <link rel="stylesheet" type="text/css" href="../css/owl.css">


  <!--<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/owl-carousel/1.3.3/owl.theme.min.css" />
  <link rel="stylesheet" type="text/css" href="http://jti.polije.ac.id/an-theme/jtipolije/assets/css" /> -->


  <link rel="stylesheet" type="text/css" href="../css/slit-slider.css">

  <!--<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.5.1/animate.min.css" />-->
  <style type="text/css">.fancybox-margin{margin-right:16px;}</style>

        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css"/>
        <style>
            /*custom css*/
            .pagination, .pager{
                margin-top: 0px
            }
            .table{
                margin-top: 20px;
            }
        </style>
        &lt;<link rel="stylesheet" type="text/css" href="../css/animate.css">&gt;


  <!--<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/devicons/1.8.0/css/devicons.min.css" />-->
  <link rel="stylesheet" type="text/css" href="../css/devicons.css">


  <link rel="stylesheet" type="text/css" href="../css/jssocials.css">

  <link rel="stylesheet" type="text/css" href="../css/jssocials-theme-flat.css">

  <link rel="stylesheet" type="text/css" href="../css/prism.css">
  <link rel="stylesheet" type="text/css" href="../css/main.css">
  <link rel="stylesheet" type="text/css" href="../css/custom.css">

  <style type="text/css">
    /*Masukan code CSS anda disini
Gunakan flag !important'*/
img#akreditasi {
  position:fixed !important;
  bottom: 20px;
    right:20px;
}
@media only screen and (max-device-width: 768px) {
  img#akreditasi {
      position:fixed !important;
      bottom: 0;
      right:20px;
      height:200px;
  }
  .a {
    size: 20px;
  }
} </style>
<script type="text/javascript" async="" src="../css/recaptcha__id.js"></script><script src="../css/modernizr-2.js"></script>
<!-- Fixed navbar -->
    <nav class="navbar navbar-default navbar-fixed-top animated-header">
      <div class="container">
        <div class="navbar-header">
          <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <a class="navbar-brand" href="#"><img src="img_admin/logodaun.png">JEMBER's<br>
          <span class="polije">tobbaco</span></a>
        </div>
        <div id="navbar" class="navbar-collapse collapse">          
          <ul class="nav navbar-nav navbar-left">
            <li><ul class="nav navbar-nav">
              <li><a href="data_para_petani.php" target="_self">Data</a></li>
              <li><a href="postingan.php" target="_self">Postingan</a></li>
              <li><a href="postingan_petani.php" target="_self">Postingan Petani</a></li>
              <li><a href="daftar_feedback.php" target="_self">Feedback</a></li>
              <li><a href="logout.php" target="_self">Log Out</a></li>
                    
          </ul>
        </div>
      </div>
    </nav>
